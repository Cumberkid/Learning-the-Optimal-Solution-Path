...
from lib.ngs.utils_ngs import find_closest

'''Calibrated Uniform Discretization'''
...
def GD_on_a_grid(lam, lam_max, epochs, loss_fn, model, test_model, optimizer, trainDataLoader,
                 ...
                 oracle=False, true_loss_list=None, 
                #  coarse_weights=None, coarse_params=None, 
                 fine_delta_lam=None, stopping_criterion=None, check_frequency=1, device="cpu"):
    ...
    for t in range(epochs):
        ...
        if true_loss_list is not None:
            if (t+1) % check_frequency == 0:
                ...
                if oracle: # check if we are within stopping criterion
...
"""Uniform discretization starts from $\lambda = 1$ and decreases $\lambda$ by $\Delta\lambda = \frac{\lambda_\text{max} - \lambda_\text{min}}{\text{# of grid}}$. The model trained on each grid point $(\lambda - \Delta\lambda)$ initializes weight with the linear weight of the model trained on the previous grid point $\lambda$."""

# do the whole naive grid search over a list of uniformly spaced lambda's
# from lam_min to lam_max
# returns a list of trained models
def uniform_discretization(lam_min, lam_max, num_grid, epochs, loss_fn, trainDataLoader,
                           data_input_dim, lr=1e-3, step_size=None, const=None, weighted_avg=False, 
                           testDataLoader=None, oracle=False, true_loss_list=None, coarse_params=None, 
                           coarse_intercepts=None, coarse_weights=None, stopping_criterion=None, 
    ...
    for lam in lambdas:
        ...
        if coarse_params is not None:
            weight, intercept = find_closest(coarse_params, coarse_weights, coarse_intercepts, lam)
            with torch.no_grad():    
