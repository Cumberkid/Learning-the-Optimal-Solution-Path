...
from lib.ngs.log_reg_solver import test
from lib.ngs.fair_reg_solver import fair_test
...
def get_losses(lam_min, lam_max, fine_delta_lam, intercepts, weights, reg_params, data_loader, loss_fn, obj=None, device='cpu'):
    ...
    for i in range(round((lam_max - lam_min) / fine_delta_lam) + 1):
        ...
        if obj == "logit":
            ...
            approx_loss = fair_test(data_loader, model, loss_fn, lam, device)
...
def get_errs(lam_min, lam_max, true_loss_list, intercepts, weights, reg_params, data_loader, loss_fn, obj=None, device='cpu'):
    fine_delta_lam = (lam_max - lam_min)/(len(true_loss_list)-1)
    if obj is None:
        print("Please enter the objective: 'logit' or 'fairness'")
        return
    losses = get_losses(lam_min, lam_max, fine_delta_lam, intercepts, weights, reg_params, data_loader, loss_fn, obj, device)
    errs = losses - true_loss_list
    return errs

# return the supremum absolute error compared to the true loss accross the solution path
def get_sup_error(lam_min, lam_max, true_loss_list, intercepts, weights, reg_params, data_loader, loss_fn, obj=None, device='cpu'):
    if obj is None:
        print("Please enter the objective: 'logit' or 'fairness'")
        return
    errs = get_errs(lam_min, lam_max, true_loss_list, intercepts, weights, reg_params, data_loader, loss_fn, obj, device)
    return max(errs)
