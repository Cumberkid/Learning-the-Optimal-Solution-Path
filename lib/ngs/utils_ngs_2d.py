...
def get_losses(fix_lam, lam_min, lam_max, fine_delta_lam, intercepts, weights, hyper_params_list, data_loader, loss_fn, device="cpu"):
...
def get_losses_2d(lam_min_2d, lam_max_2d, fine_delta_lam_2d, intercepts_2d, weights_2d, hyper_params_2d, data_loader, loss_fn, device="cpu"):
    losses_2d = []
    coarse_grid = 0

    for i in range(round((lam_max_2d[0] - lam_min_2d[0]) / fine_delta_lam_2d[0]) + 1):
        fix_lam = lam_max_2d[0] - i * fine_delta_lam_2d[0]
...
def get_errs_2d(lam_min_2d, lam_max_2d, true_loss_list_2d, intercepts_2d, weights_2d, hyper_params_2d, data_loader, loss_fn, device="cpu"):
    fine_delta_lam_2d = [(lam_max_2d[0] - lam_min_2d[0])/(len(true_loss_list_2d)-1), (lam_max_2d[0] - lam_min_2d[0])/(len(true_loss_list_2d[0])-1)]
    losses_2d = get_losses_2d(lam_min_2d, lam_max_2d, fine_delta_lam_2d, intercepts_2d, weights_2d, hyper_params_2d, data_loader, loss_fn, device)
