...
# trace_frequency is measured in number of batches. -1 means don't print
def train_bfgs(itr, init_weight, init_intercept, dataloader, model, loss_fn, optimizer, weighted_avg=False, step_size=None, const=None, device='cpu'):
    # size = len(dataloader.dataset)
    model.train()
    weight = init_weight #initialize
    intercept = init_intercept 
    ...
    for batch, (X_train, y_train) in enumerate(dataloader):
        ...
        def closure():
            nonlocal function_evaluations  # Use the nonlocal keyword to modify the counter
